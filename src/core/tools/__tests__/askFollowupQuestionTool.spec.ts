...
describe("askFollowupQuestionTool", () => {
	...
	describe("handlePartial with native protocol", () => {
		...
		it("should ignore Task.ask 'Current ask promise was ignored' errors during partial streaming", async () => {
			mockCline.ask = vi.fn().mockRejectedValue(new Error("Current ask promise was ignored (#2)"))
			...
			await expect(
				askFollowupQuestionTool.handle(mockCline, block, {
					askApproval: vi.fn(),
					handleError: vi.fn(),
					pushToolResult: mockPushToolResult,
					removeClosingTag: vi.fn((tag, content) => content || ""),
