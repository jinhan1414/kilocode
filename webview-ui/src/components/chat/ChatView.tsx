				...
				case "ask":
					...
					switch (lastMessage.ask) {
						...
						case "tool": {
							...
							const tool = (() => {
								const raw = lastMessage.text ?? ""
								if (raw.trim() === "") {
									return {} as ClineSayTool
								}

								try {
									return JSON.parse(raw) as ClineSayTool
								} catch (_) {
									// Some tool payloads may include markdown or non-JSON prefixes (e.g. "# ...", code fences).
									// Try to recover by extracting the JSON object substring.
									const firstBrace = raw.indexOf("{")
									const lastBrace = raw.lastIndexOf("}")
									if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
										try {
											return JSON.parse(raw.slice(firstBrace, lastBrace + 1)) as ClineSayTool
										} catch (_) {
											// fall through
										}
									}

									// Backwards-compat fallback used elsewhere in this file.
									if (raw.includes("updateTodoList")) {
										return { tool: "updateTodoList" } as ClineSayTool
									}
